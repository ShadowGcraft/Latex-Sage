...
\begin{center}
\sageplot[width=.4\textwidth]{plot(sinh(x), x, -5, 5)}
\sageplot[width=.4\textwidth]{plot(diff(sinh(x), x), x, -5, 5)} \\
...
\begin{center}
\sageplot[width=.4\textwidth]{plot(tanh(x), x, -5, 5)} 
\sageplot[width=.4\textwidth]{plot(diff(tanh(x), x) , x, -5, 5)} \\
...
&  =  \ln|u(x)| \\
&  =   \ln|\cosh(x)|  \quad \textrm{or } \cosh(x)>0 \; \textrm{sur l'intervalle considéré, donc}
\end{align*}
